...
const FName USBSpringArmComponent::SocketName(TEXT("SpringEndpoint"));

USBSpringArmComponent::USBSpringArmComponent(const FObjectInitializer& ObjectInitializer)
...
FRotator USBSpringArmComponent::GetDesiredRotation() const
{
	return GetComponentRotation();
}

FRotator USBSpringArmComponent::GetTargetRotation() const
{
...
void USBSpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
...
FVector USBSpringArmComponent::BlendLocations(const FVector& DesiredArmLocation, const FVector& TraceHitLocation, bool bHitSomething, float DeltaTime)
{
	return bHitSomething ? TraceHitLocation : DesiredArmLocation;
}

void USBSpringArmComponent::ApplyWorldOffset(const FVector & InOffset, bool bWorldShift)
{
...
void USBSpringArmComponent::OnRegister()
{
	Super::OnRegister();
...
void USBSpringArmComponent::PostLoad()
{
	Super::PostLoad();
}

void USBSpringArmComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
	UpdateDesiredArmLocation(bDoCollisionTest, bEnableCameraLag, bEnableCameraRotationLag, DeltaTime);
}

FTransform USBSpringArmComponent::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
bool USBSpringArmComponent::HasAnySockets() const
{
	return true;
}

void USBSpringArmComponent::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const
{
	new (OutSockets) FComponentSocketDescription(SocketName, EComponentSocketType::Socket);
}

FVector USBSpringArmComponent::GetUnfixedCameraPosition() const
{
	return UnfixedCameraPosition;
}

bool USBSpringArmComponent::IsCollisionFixApplied() const
{
